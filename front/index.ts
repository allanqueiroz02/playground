...
function mostraNome(nome: string, sobrenome: string) {
  return `${nome} ${sobrenome}`;
}

function saudacoesPara(
  fn: (nm: string, sn: string) => string,
  nomeFn: string,
  sobrenFn: string
) {
...
function getPosition<T>(arr: T[], index: number) {
  return arr[index];
}
...
function verificaMaior<T extends number | string>(v1: T, v2: T): T {
  let maior: T;
  if (+v1 > +v2) maior = v1;
  else maior = v2;
